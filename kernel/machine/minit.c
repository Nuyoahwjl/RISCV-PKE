...
// set to 1 by hart0 in s_start after pmm/vm init, used to delay other harts.
extern volatile int s_mem_init_done;

// HTIF + spike file interface should be initialized only once (by hart0).
...
void m_start(uintptr_t hartid, uintptr_t dtb) {
  ...
  if (hartid == 0) {
    ...
    while (!htif_ready) asm volatile("nop");

    // delay until hart0 finishes S-mode memory/paging init (for output ordering)
    while (!s_mem_init_done) asm volatile("nop");

    sprint("In m_start, hartid:%d\n", (int)hartid);
  }

  // per-hart interrupt frame for M-mode traps
